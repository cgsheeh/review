...
class TestEditRevision:
    ...
    @mock.patch("mozphab.repository.conduit.get_revisions")
    @mock.patch("mozphab.repository.conduit.call")
    def test_update_preserve_status_changes_planned(self, m_call, m_get_revisions):
        m_get_revisions.return_value = self._get_revisions("changes-planned")
        conduit.edit_revision(rev_id=1, preserve_status=True)
        assert (
            mock.call(
                "differential.revision.edit",
                dict(
                    transactions=[dict(type="request-review", value=True)],
                    objectIdentifier=1,
                ),
            )
            in m_call.call_args_list
        ), "`preserve_status` should call request review as normal."
        assert (
            mock.call(
                "differential.revision.edit",
                dict(
            ...
            in m_call.call_args_list
        ), "`preserve_status` should set `plan-changes`."

    @mock.patch("mozphab.repository.conduit.get_revisions")
    @mock.patch("mozphab.repository.conduit.call")
    def test_update_preserve_status_draft(self, m_call, m_get_revisions):
        m_get_revisions.return_value = self._get_revisions("draft")
        conduit.edit_revision(rev_id=1, preserve_status=True)
        assert (
            mock.call(
                "differential.revision.edit",
        ...
        assert (
            mock.call(
                "differential.revision.edit",
                dict(
                    transactions=[dict(type="draft", value=True)],
                    objectIdentifier=1,
                ),
            )
            in m_call.call_args_list
        ), "`preserve_status` should set `draft`."

    @mock.patch("mozphab.repository.conduit.get_revisions")
    @mock.patch("mozphab.repository.conduit.call")
    def test_update_preserve_status_needs_review(self, m_call, m_get_revisions):
        m_get_revisions.return_value = self._get_revisions("needs-review")
        conduit.edit_revision(rev_id=1, preserve_status=True)
        m_call.assert_called_once_with(
            "differential.revision.edit",
