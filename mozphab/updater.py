...
def check_for_updates() -> Optional[str]:
    ...
    if (
        ...
        or time.time() - config.self_last_check <= SELF_UPDATE_FREQUENCY * 60 * 60
    ):
        return
    ...
    logger.debug(
        "Versions - local: {}, PyPI: {}".format(current_version, pypi_info["version"])
    )
    ...
    logger.warning("Version %s of `moz-phab` is now available", pypi_info["version"])
